...
// ЮНІТ-ТЕСТИ ДЛЯ ЛАБОРАТОРНОЇ 3 (5 тестів)
...
namespace NetSdrClientApp.UnitTests
{
    ...
    public class NetSdrClientBasicTests
    {
        // ТЕСТ 1
        [Test]
        public void Test1_Constructor_CreatesClientSuccessfully()
        {
        ...
        // ТЕСТ 2
        [Test]
        public async Task Test2_ConnectAsync_ConnectsWhenNotConnected()
        {  
            var mockTcp = new Mock<ITcpClient>();
            var mockUdp = new Mock<IUdpClient>();
            mockTcp.Setup(x => x.Connected).Returns(false);
            
            var client = new NetSdrClient(mockTcp.Object, mockUdp.Object);
            
            await client.ConnectAsync();
            
            mockTcp.Verify(x => x.Connect(), Times.Once);
        }
        
        // ТЕСТ 3 
        [Test]
        public async Task Test3_StartIQAsync_StartsIQWhenConnected()
        {
            var mockTcp = new Mock<ITcpClient>();
            var mockUdp = new Mock<IUdpClient>();
            mockTcp.Setup(x => x.Connected).Returns(true);
        ...
        // ТЕСТ 4
        [Test]
        public async Task Test4_StopIQAsync_StopsIQWhenConnected()
        {
            var mockTcp = new Mock<ITcpClient>();
            var mockUdp = new Mock<IUdpClient>();
            mockTcp.Setup(x => x.Connected).Returns(true);
        ...
        // ТЕСТ 5
        [Test]
        public async Task Test5_ChangeFrequencyAsync_SendsFrequencyCommand()
        {
            var mockTcp = new Mock<ITcpClient>();
            var mockUdp = new Mock<IUdpClient>();
            ...
            mockTcp.Verify(x => x.SendMessageAsync(It.IsAny<byte[]>()), Times.Once);
        }
    }
}
#endif
